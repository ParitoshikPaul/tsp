<?php 
/* ================================================
	SKIN INSTAGRAM CAROUSEL
	Skin - Premium WordPress Theme, by NordWood
=================================================== */
	add_action( 'widgets_init', 'skin_instagram_carousel_init' );
	
	if ( ! function_exists( 'skin_instagram_carousel_init' ) ) :
		function skin_instagram_carousel_init() {
			register_widget( 'skin_instagram_carousel' );
		}
	endif;

	class Skin_Instagram_Carousel extends WP_Widget {
		public function __construct() {
			$widget_ops = array(
				'classname' => 'skin-widget-instagram-carousel skin-widget-instagram loading-holder promised',
				'description' => esc_html__( 'Instagram feed in carousel mod', 'skin' )
			);

			parent::__construct( 'skin_instagram_carousel', esc_html__( 'Skin Instagram Carousel', 'skin' ), $widget_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			if ( is_wp_error( $profile ) || empty( $instance['username'] ) || !$qty_in_range ) {
				...
				if ( is_array( $profile ) && !empty( $get_body ) ) {
					...
					?>
					<div class="skin-carousel swiper-container worker"><div class="swiper-wrapper"><?php					
					if ( 1 === $show_counts ) {
						for ( $i = 0; $i < $qty; $i++ ) {
							$item = $items[$i]['node'];
						?>
							<a class="item swiper-slide bgr-cover"
								href="<?php echo esc_url('http://instagram.com/p/'.$item['shortcode']); ?>"
								target="_blank"
							>
								<img src="<?php echo esc_url( $item['thumbnail_src'] ); ?>" />
						...
						?>
							<a class="item swiper-slide bgr-cover"
								href="<?php echo esc_url('http://instagram.com/p/'.$item['code']); ?>" target="_blank"
							>
								<img src="<?php echo esc_url( $item['thumbnail_src'] ); ?>" alt="<?php echo esc_attr( $item['code'] ); ?>" />
							</a>
						<?php
						}
					}
...
===================== */
		public function form( $instance ) {
			...
			if ( ( 3 > $qty ) || ( 12 < $qty ) ) {
			?>
				<p style="color: #f00;"><?php esc_html_e( 'Number of items should be between 3 and 12.', 'skin' ); ?></p>
			<?php
			}
			?>
			<p>	
